<template>
  <a-card :bordered="false" class="card-area">
    ...
    <!-- 评价弹窗 -->
    <a-modal
      v-model="evaluationModal.visible"
      title="面试评价"
      @ok="submitEvaluation"
    ...
    >
      <a-form layout="vertical">
        <a-form-item label="评价分数">
          <a-rate
            v-model="evaluationModal.form.score"
            :count="10"
            allow-half        style="font-size: 20px;"
          />
          <span>{{ evaluationModal.form.score }} 分</span>
        </a-form-item>

        <a-form-item label="满意度">
          <a-radio-group v-model="evaluationModal.form.satisfaction">
            <a-radio value="非常不满意">非常不满意</a-radio>
            <a-radio value="不满意">不满意</a-radio>
            <a-radio value="一般">一般</a-radio>
            <a-radio value="满意">满意</a-radio>
            <a-radio value="非常满意">非常满意</a-radio>
          </a-radio-group>
        </a-form-item>

        <a-form-item label="评价内容">
          <a-textarea
            v-model="evaluationModal.form.comment"
            placeholder="请输入对本次面试的评价..."
            :rows="4"
          />
        </a-form-item>
      </a-form>
...
export default {
    ...
    return {
      ...
      evaluationModal: {
        ...
        form: {
          score: 0,
          comment: '',
          satisfaction: '非常满意' // 1-非常不满意，2-不满意，3-一般，4-满意，5-非常满意
        }
      }
    }
  },
  ...
  methods: {
    showEvaluation(record) {
      this.evaluationModal.record = record
      this.evaluationModal.visible = true
    },

    // 提交评价
    submitEvaluation() {
      const { record, form } = this.evaluationModal
      ...
      this.$post('/cos/evaluate-info', {
        ...
        satisfy: form.satisfaction
      }).then(response => {
        this.$message.success('评价提交成功')
        this.evaluationModal.visible = false
